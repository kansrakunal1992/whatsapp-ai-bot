...
export default async (req, res) => {
  ...
  try {
    ...
    // Save/update business config with linked_numbers initialization
    await client.query(
      `INSERT INTO business_configs (
       ...
       ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), COALESCE($9, ARRAY[]::text[]))
       ...
       DO UPDATE SET
         ...
         linked_numbers = COALESCE(EXCLUDED.linked_numbers, business_configs.linked_numbers),
         updated_at = NOW()`,
      [
        formattedNumber,
        ...
        [formattedNumber] // Initialize linked_numbers with business number
      ]
    );
      ...
      if (process.env.TEST_MODE === 'true') {
        ...
        await client.query(
          `UPDATE business_configs 
           SET linked_numbers = array_append(
             COALESCE(linked_numbers, ARRAY[]::text[]), 
             $1
           )
        ...
        // Production configuration - use the business's own number
        whatsappPhone = formattedNumber;
