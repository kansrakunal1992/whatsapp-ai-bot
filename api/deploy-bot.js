  ...
  try {
    ...
    try {
      if (process.env.TEST_MODE === 'true') {
        ...
        // Try webhook configuration but continue if it fails
        try {
          ...
          console.warn("Sandbox webhook config may fail in trial accounts. Proceeding anyway.");
        }

        // Add sandbox number to linked_numbers
      ...
      throw new Error(
        process.env.TEST_MODE === 'true'
          ? "To activate sandbox:\n1. Send WhatsApp 'join ready' to +14155238886\n2. Wait 2 minutes\n3. Retry activation"
    ...
    return res.status(200).json({
      ...
      setupInstructions: process.env.TEST_MODE === 'true' ? [
        `1. Share sandbox number: ${whatsappPhone}`,
        `2. Users must first send "join ready" to ${whatsappPhone}`,
        `3. Test now: https://wa.me/${whatsappPhone.replace('+', '')}?text=join%20ready`,
        `4. Wait 2 minutes after joining before testing`
      ] : [
      ...
      nextSteps: [
        ...
        process.env.TEST_MODE === 'true' 
          ...
          : "Monitor your dashboard for messages"
      ],
