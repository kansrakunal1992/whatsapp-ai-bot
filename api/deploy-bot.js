...
function formatIndianNumber(rawNumber) {
  ...
  // Handle cases where number already has +91 or 91 prefix
  if (clean.startsWith('91') && clean.length === 12) {
    return `+${clean}`;
  }
  
  // Handle 10-digit numbers without prefix
  if (clean.length === 10) {
    return `+91${clean}`;
  }
  
  // Handle numbers that already include +91
  if (clean.startsWith('91') && clean.length === 12) {
    return `+${clean}`;
  }
  
  throw new Error('Invalid Indian number format. Please use 10-digit number or +91XXXXXXXXXX format');
}

// Helper: Normalize number for database queries (removes + and keeps only digits)
function normalizeNumberForQuery(number) {
...
export default async (req, res) => {
  ...
  try {
    ...
    // 3.1 Check for number conflicts in a transaction-safe way
    ...
    if (conflictCheck.rows.length > 0) {
      throw new Error(`This number is already registered to ${conflictCheck.rows[0].business_name}`);
    }

    // 3.2 UPSERT business configuration with proper conflict handling
    ...
    try {
      if (process.env.TEST_MODE === 'true') {
        ...
        const numbers = await twilioClient.incomingPhoneNumbers.list({ 
          phoneNumber: normalizeNumberForQuery(whatsappPhone) 
        });
