  ...
  try {
    ...
    try {
      if (process.env.TEST_MODE === 'true') {
        // Force sandbox number in test mode
        ...
        // Production configuration - verify number exists in Twilio first
        whatsappPhone = formattedNumber;
        const numbers = await twilioClient.incomingPhoneNumbers.list({ phoneNumber: whatsappPhone });
        if (numbers.length === 0) {
          throw new Error(`Number ${whatsappPhone} not found in Twilio account. Purchase number first.`);
        }
      ...
      throw new Error(
        process.env.TEST_MODE === 'true'
          ? "WhatsApp sandbox setup failed. Contact support."
          : `WhatsApp setup failed: ${twilioError.message}`
      );
    }
    ...
    console.error("Deployment error:", {
      ...
      numberAttempted: formattedNumber
    });
    ...
    return res.status(500).json({
      ...
      solutionSteps: [
        process.env.TEST_MODE === 'true'
          ? "1. Verify TEST_MODE=true in environment"
          : "1. Purchase number in Twilio Console",
        "2. Check number formatting",
        "3. Verify Twilio account credentials",
