...
export default async (req, res) => {
  ...
  try {
    ...
    if (!saveResult.rows[0]?.id) {
      throw new Error("Failed to save business configuration - no ID returned");
    }
    ...
    if (twilioUpdate.smsUrl !== webhookUrl) {
      throw new Error(`Twilio webhook configuration failed. Expected ${webhookUrl} but got ${twilioUpdate.smsUrl}`);
    }
