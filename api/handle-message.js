...
export default async (req, res) => {
  ...
  const {
    ...
    To: businessNumber,
    Body: message,
    MessageSid: id // Changed from messageId to id
  } = req.body;
  ...
  try {
    ...
    // 4. Log the incoming message first (using message_logs table and id)
    await client.query(`
      INSERT INTO message_logs (
        business_number, 
        customer_number, 
        message, 
        id, // Changed from message_id to id
        timestamp
      ) VALUES ($1, $2, $3, $4, NOW())
      RETURNING id
...
// Helper: Mark message as answered in DB (updated to use id instead of message_id)
async function markMessageAsAnswered(client, id) {
  ...
  await client.query(`
    ...
    SET is_answered = true,
        answered_at = NOW()
    WHERE id = $1
  `, [id]).catch(err => console.error('Error marking message as answered:', err));
}