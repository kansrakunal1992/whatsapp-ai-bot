...
export default async (req, res) => {
  ...
  try {
    ...
    // Enhanced query with duplicate-safe array handling
    const query = `
      SELECT 
        *,
        ARRAY(SELECT DISTINCT unnest(linked_numbers)) AS clean_linked_numbers
      FROM business_configs 
      WHERE ${
        process.env.TEST_MODE === 'true' 
          ? 'whatsapp_number = $1'
          : '$1 = ANY(linked_numbers)'
      }
      LIMIT 1
    `;
    ...
    // Language-specific greetings (unchanged)
    ...
    // Check for exact Q&A match (unchanged)
    ...
    // Prepare context for AI response (unchanged)
