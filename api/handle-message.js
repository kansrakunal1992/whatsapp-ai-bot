...
export default async (req, res) => {
  ...
  try {
    ...
    // 3. Fetch business config with available columns
    ...
    if (message.toLowerCase().startsWith('join')) {
      const welcomeMessage = businessConfig.language === 'Hindi' 
        ? `नमस्ते! ${businessConfig.business_name} में आपका स्वागत है। आप कैसे मदद कर ससकते हैं?`
    ...
    // 7. Prepare business context for AI using only available fields
    const businessContext = {
      ...
      // These fields are not in the current table, so we use generic values
      priceRange: 'various prices',
      services: 'our services',
      discountGroups: 'eligible customers',
